  ...
  const [initial] = useState(() => {
    try {
      ...
      return {
        ...
        raw: null,
        error:
          "Saved data could not be loaded. Saving is disabled to preserve it. Restore the browser data before continuing.",
      };
    }
  });
  ...
  const commit = (change: T | ((previous: T) => T)) => {
    try {
      ...
      if (localStorage.getItem(key) !== raw.current)
        throw Error(
          "This data changed in another tab. Refresh before saving to avoid overwriting it.",
        );
      const next =
      ...
      if (!validate(next))
        throw Error(
          "The data could not be saved because it contains invalid values.",
        );
      ...
      setError(
        e instanceof Error
          ? e.message
          : "Saving failed. Check browser storage and try again.",
      );
      return false;
    }
  };
