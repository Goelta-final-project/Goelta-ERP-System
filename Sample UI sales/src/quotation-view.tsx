import { calculateTotals, canTransition, roundMoney } from './sales-logic';
...
export function QuotationDetails({ quotation, onStatusChange }: { quotation?: Quote; onStatusChange: (id: string, status: Quote['status'], recipients?: Quote['recipients']) => boolean }) {
  ...
  const totals = q.lines ? calculateTotals(q.lines, q.discount, q.taxRate) : undefined;
  ...
  return <Box sx={{ p: { xs: 3, md: 5 }, maxWidth: 1400 }}>
    ...
    <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" spacing={2} sx={{ mb: 3 }}><Box><Typography variant="overline" color="primary">Quotation details</Typography><Stack direction="row" alignItems="center" spacing={2} flexWrap="wrap"><Typography variant="h4" fontWeight={700}>{q.id}</Typography><Chip label={q.status} color={colors[q.status]} size="small" />{decisionDate && <Typography variant="body2" color="text.secondary">{q.status} on {decisionDate}</Typography>}<Button size="small" variant="outlined" disabled={!(['Draft', 'Accepted', 'Rejected', 'Hold'] as const).some(status => canTransition(q.status, status))} onClick={event => setStatusMenuAnchor(event.currentTarget)}>Set status</Button><Menu anchorEl={statusMenuAnchor} open={!!statusMenuAnchor} onClose={() => setStatusMenuAnchor(null)}>{(['Draft', 'Accepted', 'Rejected', 'Hold'] as const).filter(status => canTransition(q.status, status)).map(status => <MenuItem key={status} onClick={() => { if (onStatusChange(q.id, status)) setStatusMenuAnchor(null); }}>{status}</MenuItem>)}</Menu></Stack></Box><Button component={Link} to="/sales/quotations" variant="outlined" sx={{ alignSelf: 'flex-start' }}>Back to quotations</Button></Stack>
    {q.status === 'Draft' && <Button component={Link} to={`/sales/quotations/${encodeURIComponent(q.id)}/edit`} variant="contained" sx={{ mb: 2 }}>Edit draft</Button>}
    <Box sx={{ mb: 3 }}><QuotationPdfActions key={q.id} quotation={q} onSent={recipients => onStatusChange(q.id, 'Sent', recipients)} /></Box>
    {q.isDemo && <Alert severity="info" sx={{ mb: 3 }}>Demo quotation. The item breakdown is sample data.</Alert>}
    <Grid container spacing={3}>
      <Grid item xs={12} md={8}><Stack spacing={3}>
        <Paper variant="outlined" sx={{ p: 3 }}><Typography variant="h6" gutterBottom>Quotation information</Typography><Grid container spacing={2}><Grid item xs={12}><Detail label="Title" value={q.title} /></Grid><Grid item xs={12}><Detail label="Description" value={q.description} /></Grid><Grid item xs={12} sm={6}><Detail label="Quotation number" value={q.id} /></Grid><Grid item xs={12} sm={6}><Detail label="Status" value={q.status} /></Grid><Grid item xs={12} sm={6}><Detail label="Quotation date" value={q.date} /></Grid><Grid item xs={12} sm={6}><Detail label="Expiry date" value={q.expiry} /></Grid></Grid></Paper>
        ...
        <Paper variant="outlined" sx={{ overflow: 'hidden' }}><Typography variant="h6" sx={{ p: 3 }}>Items</Typography>{q.lines?.length ? <TableContainer><Table size="small" aria-label="Quotation items"><TableHead sx={{ bgcolor: 'grey.50' }}><TableRow><TableCell>Item</TableCell><TableCell>Description</TableCell><TableCell>Unit</TableCell><TableCell align="right">Quantity</TableCell><TableCell align="right">Unit price</TableCell><TableCell align="right">Amount</TableCell></TableRow></TableHead><TableBody>{q.lines.map((line, index) => <TableRow key={index}><TableCell>{line.itemNo || index + 1}</TableCell><TableCell sx={{ minWidth: 200, py: 2 }}>{line.description}{Object.entries(line.attributes || {}).map(([key, value]) => <Typography key={key} variant="caption" display="block" color="text.secondary">{key}: {String(value) || '—'}</Typography>)}</TableCell><TableCell>{line.unit || '—'}</TableCell><TableCell align="right">{line.quantity}</TableCell><TableCell align="right" sx={{ whiteSpace: 'nowrap' }}>{money(line.unitPrice)}</TableCell><TableCell align="right" sx={{ whiteSpace: 'nowrap' }}>{money(roundMoney(line.quantity * line.unitPrice))}</TableCell></TableRow>)}</TableBody></Table></TableContainer> : <Typography sx={{ px: 3, pb: 3 }} color="text.secondary">No item breakdown was recorded.</Typography>}</Paper>
      </Stack></Grid>
