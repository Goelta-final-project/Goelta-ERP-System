...
export function QuotationPdfActions({ quotation, onSent }: { quotation: Quote; onSent: () => void }) {
...
function QuotationEmailDialog({ quotation: q, onClose, onSent }: { quotation: Quote; onClose: () => void; onSent: () => void }) {
  ...
  const [to, setTo] = useState(q.recipients?.find(r => r.role === 'To')?.email || options.find(o => o.id === defaults?.to)?.email || q.customerEmail || '');
  const [cc, setCc] = useState<string[]>(q.recipients?.filter(r => r.role === 'CC').map(r => r.email) || options.filter(o => defaults?.cc.includes(o.id)).map(o => o.email));
  ...
  const canShare = !!pdf && typeof navigator.canShare === 'function' && typeof navigator.share === 'function' && navigator.canShare({ files: [pdf.file] });
  ...
  const share = async () => {
    ...
    try { await navigator.share({ files: [pdf.file], title: subject, text: body }); setNotice('PDF handed to the selected app. Review recipients and complete sending there.'); }
